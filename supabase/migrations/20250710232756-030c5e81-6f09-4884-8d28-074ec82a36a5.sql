...
-- Fix feedback sessions policy - restrict to proper token-based access
DROP POLICY IF EXISTS "Users can view own feedback sessions via token" ON public.feedback_sessions;

CREATE POLICY "Token-based feedback session access" 
ON public.feedback_sessions 
FOR SELECT 
USING (
  -- Only allow access if the request includes a valid token
