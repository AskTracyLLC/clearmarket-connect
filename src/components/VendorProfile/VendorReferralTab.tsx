...
const VendorReferralTab = () => {
  ...
  const [newRepInitials, setNewRepInitials] = useState("");
  ...
  const handleAddReferral = () => {
    if (!newRepName.trim() || !newRepInitials.trim()) {
      toast({
        title: "Missing Information",
        description: "Please enter both rep name and initials",
        variant: "destructive",
      });
      return;
    }

    addReferral(newRepName.trim(), newRepInitials.trim());
    setNewRepName("");
    setNewRepInitials("");
    toast({
      title: "Referral Added",
      description: "Rep has been added to your referral tracking",
    });
  };
      ...
      <Card>
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="space-y-2">
              <Label htmlFor="rep-initials">Rep Initials</Label>
              <Input
                id="rep-initials"
                value={newRepInitials}
                onChange={(e) => setNewRepInitials(e.target.value)}
                placeholder="J.D."
              />
            </div>
