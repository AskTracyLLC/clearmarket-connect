...
export const ComprehensiveSignupForm = ({ onSubmit, loading }: ComprehensiveSignupFormProps) => {
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="space-y-3">
        ...
        <div className="grid grid-cols-2 gap-4">
          <button
            type="button"
            onClick={() => setFormData(prev => ({ ...prev, userRole: 'field-rep' }))}
            className={cn(
              "relative p-6 rounded-lg border-2 transition-all text-center",
              formData.userRole === 'field-rep'
          ...
          <button
            type="button"
            onClick={() => setFormData(prev => ({ ...prev, userRole: 'vendor' }))}
            className={cn(
              "relative p-6 rounded-lg border-2 transition-all text-center",
      ...
      <div className="space-y-2">
        ...
        <Select
          ...
          onValueChange={(value) => setFormData(prev => ({ ...prev, primaryState: value }))}
        >
      ...
      <div className="space-y-3">
        ...
        <div className="space-y-2 p-4 rounded-lg border border-border bg-card">
          ...
          {formData.workTypes && formData.workTypes.length === 0 && (
            <p className="text-sm text-muted-foreground mt-2">Please select at least one work type</p>
          )}
        </div>
      </div>
      ...
      <div className="space-y-2">
        ...
        <Textarea
          ...
          onChange={(e) => setFormData(prev => ({ ...prev, interestedFeatures: e.target.value }))}
          className="min-h-[80px] bg-background resize-none"
        />
      </div>
